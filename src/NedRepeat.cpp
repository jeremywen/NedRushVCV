...
struct NedRepeatWidget : ModuleWidget {
	NedRepeatWidget(NedRepeat *module);
	~NedRepeatWidget() {
	}
	void appendContextMenu(Menu *menu) override;
};

NedRepeatWidget::NedRepeatWidget(NedRepeat *module) {
	setModule(module);
	box.size = Vec(RACK_GRID_WIDTH * 8, RACK_GRID_HEIGHT);

	setPanel(createPanel(
		asset::plugin(pluginInstance, "res/NedRepeat.svg"),
		asset::plugin(pluginInstance, "res/NedRepeat.svg")
	));
	...
	addParam(createParamCentered<RoundBlackKnob>(Vec(40.f, 100.f), module, NedRepeat::REPEAT_LENGTH_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(80.f, 100.f), module, NedRepeat::REPEAT_LENGTH_CV_INPUT));
	addParam(createParamCentered<RoundBlackKnob>(Vec(40.f, 170.f), module, NedRepeat::OFFSET_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(80.f, 170.f), module, NedRepeat::OFFSET_CV_INPUT));

	addInput(createInputCentered<PJ301MPort>(Vec(25.f, 270.f), module, NedRepeat::AUDIO_L_INPUT));
	addInput(createInputCentered<PJ301MPort>(Vec(25.f, 315.f), module, NedRepeat::AUDIO_R_INPUT));
	addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x - 25.f, 270.f), module, NedRepeat::AUDIO_L_OUTPUT));
	addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x - 25.f, 315.f), module, NedRepeat::AUDIO_R_OUTPUT));
}

void NedRepeatWidget::appendContextMenu(Menu *menu) {
	MenuLabel *spacerLabel = new MenuLabel();
	menu->addChild(spacerLabel);
}

Model *modelNedRepeat = createModel<NedRepeat, NedRepeatWidget>("NedRepeat");
