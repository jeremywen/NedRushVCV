...
struct NedKickWidget : ModuleWidget {
	NedKickWidget(NedKick *module);
	~NedKickWidget(){
	}
	void appendContextMenu(Menu *menu) override;
};

NedKickWidget::NedKickWidget(NedKick *module) {
	setModule(module);
	box.size = Vec(RACK_GRID_WIDTH * 8, RACK_GRID_HEIGHT);

	setPanel(createPanel(
		asset::plugin(pluginInstance, "res/NedKick.svg"),
		asset::plugin(pluginInstance, "res/NedKick.svg")
	));
	...
	// Trigger input
	addInput(createInputCentered<PJ301MPort>(Vec(25, 310.f), module, NedKick::TRIGGER_INPUT));

	// Parameters and CV inputs
	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 67.f), module, NedKick::FREQUENCY_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 67.f), module, NedKick::FREQUENCY_CV_INPUT));

	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 107.f), module, NedKick::SWEEPAMT_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 107.f), module, NedKick::SWEEPAMT_CV_INPUT));

	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 147.f), module, NedKick::FILTER_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 147.f), module, NedKick::FILTER_CV_INPUT));

	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 187.f), module, NedKick::DRIVE_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 187.f), module, NedKick::DRIVE_CV_INPUT));

	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 227.f), module, NedKick::SWEEPDECAY_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 227.f), module, NedKick::SWEEPDECAY_CV_INPUT));

	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 267.f), module, NedKick::DECAY_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 267.f), module, NedKick::DECAY_CV_INPUT));

	// Audio output
	addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x - 25, 310.f), module, NedKick::AUDIO_OUTPUT));
}

void NedKickWidget::appendContextMenu(Menu *menu) {
	MenuLabel *spacerLabel = new MenuLabel();
	menu->addChild(spacerLabel);
}

Model *modelNedKick = createModel<NedKick, NedKickWidget>("NedKick");
