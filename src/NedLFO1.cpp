...
struct NedLFO1Widget : ModuleWidget {
	NedLFO1Widget(NedLFO1 *module);
	~NedLFO1Widget(){ 
	}
	void appendContextMenu(Menu *menu) override;
};

NedLFO1Widget::NedLFO1Widget(NedLFO1 *module) {
	setModule(module);
	box.size = Vec(RACK_GRID_WIDTH * 8, RACK_GRID_HEIGHT);

	setPanel(createPanel(
		asset::plugin(pluginInstance, "res/NedLFO1.svg"), 
		asset::plugin(pluginInstance, "res/NedLFO1.svg")
	));
	...
	// Add parameters
	addParam(createParamCentered<RoundBlackKnob>(Vec(40.f, 80.f), module, NedLFO1::TYPE_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(80.f, 80.f), module, NedLFO1::TYPE_CV_INPUT));
	addParam(createParamCentered<RoundBlackKnob>(Vec(40.f, 150.f), module, NedLFO1::FREQUENCY_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(80.f, 150.f), module, NedLFO1::FREQUENCY_CV_INPUT));
	addParam(createParamCentered<RoundBlackKnob>(Vec(40.f, 220.f), module, NedLFO1::CHANCE_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(80.f, 220.f), module, NedLFO1::CHANCE_CV_INPUT));
	addParam(createParamCentered<CKSS>(Vec(60.f, 270.f), module, NedLFO1::OUTPUT_RANGE_PARAM));

	// Add output port
	addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x / 2, 320.f), module, NedLFO1::LFO_OUTPUT));
}

void NedLFO1Widget::appendContextMenu(Menu *menu) {
	MenuLabel *spacerLabel = new MenuLabel();
	menu->addChild(spacerLabel);
}

Model *modelNedLFO1 = createModel<NedLFO1, NedLFO1Widget>("NedLFO1");
