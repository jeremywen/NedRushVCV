...
struct HelpDelayWidget : ModuleWidget {
	HelpDelayWidget(HelpDelay *module);
	~HelpDelayWidget(){ 
	}
	void appendContextMenu(Menu *menu) override;
};

HelpDelayWidget::HelpDelayWidget(HelpDelay *module) {
	setModule(module);
	box.size = Vec(RACK_GRID_WIDTH * 8, RACK_GRID_HEIGHT);

	setPanel(createPanel(
		asset::plugin(pluginInstance, "res/HelpDelay.svg"), 
		asset::plugin(pluginInstance, "res/HelpDelay.svg")
	));
	...
	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 80.f), module, HelpDelay::FEEDBACK_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 80.f), module, HelpDelay::FEEDBACK_CV_INPUT));

	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 130.f), module, HelpDelay::DRYWET_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 130.f), module, HelpDelay::DRYWET_CV_INPUT));

	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 180.f), module, HelpDelay::TIME_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 180.f), module, HelpDelay::TIME_CV_INPUT));

	addParam(createParamCentered<RoundBlackKnob>(Vec(35.f, 230.f), module, HelpDelay::PLAYBACKRATE_PARAM));
	addInput(createInputCentered<PJ301MPort>(Vec(65.f, 230.f), module, HelpDelay::PLAYBACKRATE_CV_INPUT));

	// Add input and output ports
	addInput(createInputCentered<PJ301MPort>(Vec(25, 300.f), module, HelpDelay::AUDIO_INPUT));
	addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x - 25, 300.f), module, HelpDelay::AUDIO_OUTPUT));
}

void HelpDelayWidget::appendContextMenu(Menu *menu) {
	MenuLabel *spacerLabel = new MenuLabel();
	menu->addChild(spacerLabel);
}

Model *modelHelpDelay = createModel<HelpDelay, HelpDelayWidget>("HelpDelay");
